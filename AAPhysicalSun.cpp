...
CAAPhysicalSunDetails CAAPhysicalSun::Calculate(double JD, bool bHighPrecision)
{
  ...
  eta = CAACoordinateTransformation::MapTo0To2PIRange(eta);
  double mappedDifference = CAACoordinateTransformation::MapTo0To2PIRange(SunLong - K);
  
  if (fabs(eta - mappedDifference) < 0.1)
	  eta += M_PI;
